  //////////////////////////////////
 // interface: remoteStorage object //
///////////////////////////////////

if(window.remoteStorageOnSync === undefined) {//test with 'window.' in front to avoid exception
  remoteStorageOnSync = function(){}
}
if(window.remoteStorageScope === undefined) {
  remoteStorageScope = location.host
}
...
//  document.getElementById('remoteStorage').innerHTML = 
...
function initSyncStorage(){
  ...
  function pull() {
    remoteEnd.get( 'index', function( result ){
      if(result.value){
        var keys = result.value
        for(key in keys) {
          remoteEnd.get( key, function( result ){
  ...
  function push() {
    remoteEnd.get( 'index', function( result ){
      ...
      for(var i=0; i<localStorage.length; i++) {
        ...
        remoteEnd.set( key, localStorage.getItem(key), function( result ){
        })
        keys[key]= (new Date()).getTime()
      }
      remoteEnd.set( 'index', keys, function( result ){})
    })
  }
  ...
  if(sessionStr) {
    ...
    if(session.storage) {
      if( session.storage.storageType == 'http://unhosted.org/spec/dav/0.1' ){
        ...
        remoteStorage.error = 'unsupported remote storage type '+ remoteStorageType
      }
    }
  }
  return (
