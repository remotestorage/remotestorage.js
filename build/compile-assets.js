
var fs = require('fs');
var path = require('path');
var child_process = require('child_process');

var argv = process.argv.slice(2);
var sourcePath = argv[0];
var targetPath = argv[1];

if(! (sourcePath && targetPath)) {
  console.error('Usage: ' + process.argv.slice(0, 2).join(' ') + ' SOURCE_DIR TARGET_FILE');
  return;
}

var output = '/** THIS FILE WAS GENERATED BY build/compile-assets.js. DO NOT CHANGE IT MANUALLY, BUT INSTEAD CHANGE THE ASSETS IN assets/. **/\n';
output += 'define([], function() {\n'
output += '  return {';

var sources = fs.readdirSync(sourcePath);
var sourcesDone = 0;

function done() {
  output = output.replace(/,$/, '');
  output += '\n}\n});\n';
  fs.writeFileSync(targetPath, output);
}

function append(key, value) {
  output += '\n    ' + key + ": '" + value.replace(/'/g, "\\'") + "',";
  sourcesDone++;
  if(sourcesDone === sources.length) {
    done();
  }
}

sources.forEach(function(fileName) {
  if(fileName.match(/~$/)) {
    return;
  }
  var parts = fileName.split('.');
  var name = parts[0];
  var ext = parts[1];
  var fullPath = path.join(sourcePath, fileName);
  var content;
  if(ext == 'png') {
    content = 'data:image/png;base64,' + fs.readFileSync(fullPath, 'base64');
  } else if(ext == 'svg') {
    // apt-get install python-scour # (or download from http://www.codedread.com/scour/)
    var scour = child_process.spawn('scour', ['--indent=none']);
    fs.createReadStream(fullPath).pipe(scour.stdin);
    var svgData = '';
    scour.stdout.on('data', function(data) {
      svgData += data;
    });
    scour.stdout.on('end', function() {
      append(name, 'data:image/svg+xml;base64,' + new Buffer(svgData, 'utf-8').toString('base64'));
    });
    return;
  } else {
    content = fs.readFileSync(fullPath, 'ascii').replace(/\n/g, ' ');
  }

  if(ext == 'css') {
    content = content.
      replace(/\s+/g, ' ').
      replace(/\s?\{\s?/g, '{').
      replace(/\s?\}\s?/g, '}').
      replace(/\n/g, '').
      replace(/;\s/g, ';').
      replace(/:\s/g, ':');
  }

  append(name, content);
});
