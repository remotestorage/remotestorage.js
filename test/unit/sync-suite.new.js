...
define(['bluebird', 'test/helpers/mocks', 'requirejs'], function(Promise, mocks, requirejs) {
  ...
  suites.push({
    ...
    tests: [
      ...
      {
        desc: "get with maxAge requirement ignores maxAge if remote is not connected",
        run: function(env, test) {
          env.rs.remote.connected = false;

          env.rs.local.setNodes({
            '/surfoffice/board': {
              path: '/surfoffice/board',
              common: { body: 'longboard', contentType: 'text/plain', revision: '987', timestamp: 1234567890123 },
              local: { body: false, timestamp: 1234567891000 }
            }
          }).then(function() {
            env.rs.local.get('/surfoffice/board', false, env.rs.sync.queueGetRequest.bind(env.rs.sync)).then(function() {
              test.done();
            }, function(err) {
              test.fail();
            });
      ...
      // {
      //   desc: "get with maxAge requirement ignores maxAge if remote is not online",
      //   run: function (env, test) {
